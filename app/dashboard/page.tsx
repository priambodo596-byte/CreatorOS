  ...
  return (
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      ...
      {yt.error && !yt.loading && (
        <Card className="glass p-5">
          <div className="flex items-center gap-3">
            <AlertCircle className={`h-5 w-5 ${yt.connectionState === 'permission_missing' || yt.connectionState === 'token_expired' ? 'text-warning' : 'text-muted-foreground'}`} />
            <div>
              {yt.connectionState === 'permission_missing' ? (
                <>
                  <p className="text-sm font-medium">YouTube Analytics permission is required</p>
                  <p className="text-xs text-muted-foreground">Please reconnect your YouTube account to grant analytics access.</p>
                  <Link href="/dashboard/settings" className="mt-1 inline-block text-xs text-primary hover:underline">Go to Settings to reconnect</Link>
                </>
              ) : yt.connectionState === 'token_expired' ? (
                <>
                  <p className="text-sm font-medium">YouTube token needs refresh</p>
                  <p className="text-xs text-muted-foreground">{yt.error}</p>
                  <button onClick={() => yt.refresh()} className="mt-1 text-xs text-primary hover:underline">Retry</button>
                </>
              ) : yt.connectionState === 'api_error' ? (
                <>
                  <p className="text-sm font-medium">YouTube API error</p>
                  <p className="text-xs text-muted-foreground">{yt.error}</p>
                  <button onClick={() => yt.refresh()} className="mt-1 text-xs text-primary hover:underline">Retry</button>
                </>
              ) : yt.connectionState === 'connected_with_data' ? (
                <>
                  <p className="text-sm font-medium">Showing cached data from last sync</p>
                  <p className="text-xs text-muted-foreground">{yt.error}</p>
                  <button onClick={() => yt.refresh()} className="mt-1 text-xs text-primary hover:underline">Retry live fetch</button>
                </>
              ) : (
                <>
                  <p className="text-sm font-medium">YouTube live data unavailable</p>
                  <p className="text-xs text-muted-foreground">{yt.error}</p>
                </>
              )}
            </div>
          ...
          <Card className="glass p-5">
            ...
            {chartData.length > 0 ? (
              ...
              <div className="flex h-[260px] flex-col items-center justify-center gap-2 text-sm text-muted-foreground">
                {yt.connectionState === 'not_connected'
                  ? 'Connect YouTube to see analytics'
                  : yt.connectionState === 'connected_loading'
                    ? 'Loading analytics data...'
                    : yt.connectionState === 'permission_missing'
                      ? 'YouTube Analytics permission is required. Please reconnect.'
                      : yt.connectionState === 'token_expired'
                        ? 'Refreshing YouTube token...'
                        : yt.connectionState === 'api_error'
                          ? 'Unable to fetch analytics data right now.'
                          : 'Your YouTube account is connected, but analytics data is not available yet.'}
                {yt.connectionState === 'not_connected' || yt.connectionState === 'permission_missing' ? (
                  <Link href="/dashboard/settings" className="text-primary hover:underline">
                    {yt.connectionState === 'not_connected' ? 'Connect YouTube' : 'Reconnect YouTube'}
                  </Link>
                ) : null}
              </div>
            )}
              ...
              <div className="flex h-[200px] flex-col items-center justify-center gap-2 text-sm text-muted-foreground">
                {yt.connectionState === 'not_connected'
                  ...
                  : yt.connectionState === 'connected_loading'
                    ...
                    : yt.connectionState === 'permission_missing'
                      ? 'Analytics permission required. Please reconnect.'
                      : yt.connectionState === 'token_expired'
                        ? 'Refreshing YouTube token...'
                        : yt.connectionState === 'api_error'
                          ? 'Unable to fetch traffic sources right now.'
                          : 'Your YouTube account is connected, but traffic data is not available yet.'}
                {yt.connectionState === 'not_connected' || yt.connectionState === 'permission_missing' ? (
                  <Link href="/dashboard/settings" className="text-primary hover:underline">
                    {yt.connectionState === 'not_connected' ? 'Connect YouTube' : 'Reconnect YouTube'}
                  </Link>
