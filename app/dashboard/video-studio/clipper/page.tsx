...
export default function AIClipperPage() {
  ...
  const handleSaveClip = useCallback(
    async (clip: GeneratedClip) => {
      ...
      try {
        ...
        // Upload source video to clips storage bucket
        ...
        if (clip.sourceVideoUrl) {
          ...
          if (isYouTubeUrl(clip.sourceVideoUrl)) {
            toast({
              title: 'Cannot download from YouTube',
              description: 'A YouTube watch URL is not a direct video file. The clip will be saved without a video file.',
            });
          } else {
            const videoRes = await fetch(clip.sourceVideoUrl);
            if (videoRes.ok) {
              const videoBlob = await videoRes.blob();
              if (videoBlob.size > 0 && videoBlob.type.startsWith('video/')) {
                const fileExt = clip.sourceVideoUrl.split('.').pop()?.split('?')[0] || 'mp4';
                const fileName = `${userId}/clips/${Date.now()}_${clip.id}.${fileExt}`;
        ...
        toast({
          title: 'Clip saved',
          description: outputStoragePath
            ? `"${clip.title}" saved with video file to your clips library.`
            : `"${clip.title}" saved to your clips library.`,
        });
      } catch (err) {
        setGeneratedClips((prev) =>
  ...
  const navigateToPublishing = async (clip: GeneratedClip) => {
    try {
      const { data: authData } = await supabase.auth.getUser();
      const userId = authData.user?.id;
      if (!userId) throw new Error('Not authenticated');

      const savedClip = existingClips.find(
        (c) =>
          c.source_video_id === (selectedVideo?.video_id || selectedVideo?.id) &&
          c.title === clip.title,
      );

      let videoStoragePath: string | null = savedClip?.output_storage_path ?? null;
      let videoUrl: string | null = savedClip?.output_url ?? null;
      ...
      if (!videoStoragePath && clip.sourceVideoUrl && !isYouTubeUrl(clip.sourceVideoUrl)) {
        try {
          const videoRes = await fetch(clip.sourceVideoUrl);
          if (videoRes.ok) {
            const videoBlob = await videoRes.blob();
            if (videoBlob.size > 0 && videoBlob.type.startsWith('video/')) {
              const fileExt = clip.sourceVideoUrl.split('.').pop()?.split('?')[0] || 'mp4';
              const fileName = `${userId}/${Date.now()}_${clip.id}.${fileExt}`;
              const { error: uploadErr } = await supabase.storage
                .from('clips')
                .upload(fileName, videoBlob);
              if (!uploadErr) {
          ...
          // Non-critical: draft will be created without a storage path
        }
      }

      if (clip.sourceVideoUrl && isYouTubeUrl(clip.sourceVideoUrl) && !videoStoragePath) {
        toast({
          title: 'Cannot publish from YouTube URL',
          description: 'A YouTube watch URL is not a video file. Upload the video file from the Publishing Center to publish.',
          variant: 'destructive',
        });
      }
      ...
      toast({
        title: 'Draft created',
        description: videoStoragePath
          ? 'Opening publishing center...'
          : 'Draft created without a video file. Upload a video file from the Publishing Center before publishing.',
      });

      const params = new URLSearchParams({ draft: data.id });
