...
export default function DraftContentPage() {
  ...
  async function handleSchedule() {
    if (!scheduleTarget || !scheduleDate) {
      toast({ title: "Please select a date/time", variant: "destructive" });
      return;
    }
    try {
      await updateContent(scheduleTarget.id, {
        status: "scheduled",
  ...
  async function handleBulkPublish() {
    ...
    try {
      ...
      toast({ title: `${selectedIds.length} drafts published` });
      setSelectedIds([]);
      load();
    } catch {
  ...
  async function handleBulkArchive() {
    ...
    try {
      ...
      toast({ title: `${selectedIds.length} drafts archived` });
      setSelectedIds([]);
      load();
    } catch {
  ...
  async function handleBulkDelete() {
    ...
    try {
      ...
      toast({ title: `${selectedIds.length} drafts moved to trash` });
      setSelectedIds([]);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      <PageHeader
        title="Drafts"
        description="Continue working on your unfinished content."
        actions={
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Video}
          title="No drafts"
          description="You don't have any drafts yet. Create your first piece of content to get started."
          action={
            <Link href="/dashboard/content">
              <Button size="sm">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Schedule Draft</AlertDialogTitle>
            <AlertDialogDescription>
              Choose a date and time to publish &quot;{scheduleTarget?.title}&quot;.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
            <Input
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Archive Draft?</AlertDialogTitle>
            <AlertDialogDescription>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem asChild>
          <Link href={`/dashboard/content`}>Edit</Link>
        </DropdownMenuItem>
  ...
  return (
    <Card className="glass glass-hover flex items-center gap-4 p-3">
      ...
      <div className="flex gap-2">
        ...
        <DraftRowActions
          {...{ item, onPublish, onSchedule, onDuplicate, onArchive, onDelete }}
        />
      </div>
    </Card>
  );
