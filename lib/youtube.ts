...
// Sends the user's Supabase JWT to the edge function. The edge function
// extracts the user ID from the JWT, looks up the YouTube tokens server-side,
// refreshes them if needed, and calls the YouTube API. Tokens never reach
// the browser.
...
async function callAnalyticsEdgeFunction(action: string, params?: Record<string, unknown>) {
  ...
  if (token) {
    ...
    throw new YouTubeApiError('Authentication required', 401);
  }

  const res = await fetch(`${supabaseUrl}/functions/v1/youtube-analytics`, {
    method: 'POST',
    headers,
  ...
  if (!res.ok) {
    const message = data.error || `YouTube API request failed (${res.status})`;
...
export async function exchangeCode(code: string, userId: string) {
  return callOAuthEdgeFunction('youtube-oauth', { action: 'callback', code, userId });
}

async function callOAuthEdgeFunction(name: string, body: unknown) {
...
// ─── Live API Functions (via edge function, tokens handled server-side) ──────

export async function fetchChannelStats(): Promise<ChannelStats> {
  const data = await callAnalyticsEdgeFunction('channel-stats');

  const channel = data.items?.[0];
  if (!channel) throw new YouTubeApiError('No channel data found', 404);
...
export async function fetchAnalytics(
  ...
  const data = await callAnalyticsEdgeFunction('analytics', { startDate, endDate });
...
export async function fetchVideos(maxResults = 10): Promise<VideoInfo[]> {
  const data = await callAnalyticsEdgeFunction('videos', { maxResults });
...
export async function fetchTrafficSources(): Promise<{ source: string; views: number }[]> {
  const data = await callAnalyticsEdgeFunction('traffic-sources');
...
export async function fetchTopCountries(): Promise<{ country: string; views: number }[]> {
  const data = await callAnalyticsEdgeFunction('top-countries');
...
export async function triggerFullSync(
  ...
  const res = await fetch(`${supabaseUrl}/functions/v1/youtube-sync`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
      userId,
