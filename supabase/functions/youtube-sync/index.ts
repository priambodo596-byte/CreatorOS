...
async function refreshAccessToken(refreshToken: string): Promise<string> {
  ...
  console.log("[youtube-sync] Refresh diagnostics:", {
...
async function supabaseUpsert(table: string, rows: unknown[], onConflict: string) {
  ...
  if (!res.ok) {
    const err = await res.text();
    console.error(`[sync] Upsert to ${table} failed (${res.status}): ${err}`);
    throw new Error(`Upsert to ${table} failed (${res.status}): ${err}`);
  }
  console.log(`[sync] Upserted ${rows.length} rows to ${table}`);
}
...
async function syncAnalytics(
  ...
  qs.set("start-date", startDate);
  qs.set("end-date", endDate);
  qs.set("metrics", metrics);
  qs.set("dimensions", "day");
  qs.set("sort", "day");
  ...
  try {
    ...
    if (testRes.status === 401 && conn.refreshToken) {
      ...
      const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
        ...
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: new URLSearchParams({
          client_id: getOAuthCredentials().clientId,
          client_secret: getOAuthCredentials().clientSecret,
      ...
      if (!tokenRes.ok) {
        ...
        console.error(`[sync] Token refresh failed: ${errText}`);
        return json({ error: `Token refresh failed: ${errText}` }, 401);
      }
    ...
    console.error("[sync] Fatal error:", err instanceof Error ? { message: err.message, stack: err.stack } : err);
    if (syncLogId) {
      await fetch(`${SUPABASE_URL}/rest/v1/youtube_sync_logs?id=eq.${syncLogId}`, {
        method: "PATCH",
        headers: {
