...
async function fetchVideoFile(supabase: ReturnType<typeof createClient>, draft: DraftRow): Promise<ArrayBuffer> {
  ...
  if (error || !data) {
    console.error("[youtube-publish] Storage download failed:", {
      bucket,
      path: draft.video_storage_path,
      error: error?.message || "no data returned",
    });
    throw new Error(`Failed to download video from Supabase Storage: ${error?.message || "no data"}`);
  }

  const videoData = await data.arrayBuffer();
  const mimeType = data.type || "unknown";
  ...
  console.log("[youtube-publish] Video file retrieved:", {
    ...
    download_url: `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${draft.video_storage_path}`,
  });
