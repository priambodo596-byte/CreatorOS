...
async function refreshAccessToken(refreshToken: string): Promise<{ access_token: string; expires_in: number }> {
  ...
  console.log("[youtube-trending] Refresh diagnostics:", {
  ...
  if (!res.ok) {
    ...
    console.error("[youtube-trending] Token refresh failed:", {
  ...
  try {
    ...
    if (!token && userId) {
      ...
      if (conn) {
        ...
        try {
          ...
          console.warn("[trending] Token refresh failed:", refreshErr instanceof Error ? refreshErr.message : refreshErr);
        }
      }
    }
    ...
    const videos = items.map((item) => {
      ...
      const growthRate = viewCount > 0
        ? Math.round((viewsPerDay / viewCount) * 100 * 30)
        : 0;
    ...
    console.error("[youtube-trending] Error:", err instanceof Error ? { message: err.message, stack: err.stack } : err);
